...
public class NoticeCommands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length != 0 && !args[0].equalsIgnoreCase("help")) {
			if (args[0].equalsIgnoreCase("version")) {
				...
				success = onModifyCommand(sender, cmd, label, args);
	...
	public boolean onHelpCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender.hasPermission("notice.admin")) {
			if (args.length <= 1) {
				...
				sender.sendMessage("§6/notice modify <§oindex§6> <§omessage§6>§f: Modify message of the index.");
	...
	public boolean onVersionCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender.hasPermission("notice.admin")) {
			if (args.length == 1) {
				sender.sendMessage(ChatColor.AQUA + "[NOTICE] " + ChatColor.GREEN + "v0.0.2");
			} else {
				return false;
			}
		} else {
	...
	public boolean onInfoCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender.hasPermission("notice.admin")) {
			if (args.length == 1) {
				...
				sender.sendMessage(ChatColor.GOLD + "Version: " + ChatColor.WHITE + "v0.0.2");
			...
			if (args.length > 2) {
				try {
					...
					if (index > 0 && index <= plugin.numberOfMessages() + 1) {
						...
						message.deleteCharAt(message.lastIndexOf(" "));
						plugin.addToNoticeMessage(message.toString(), index);
	...
	// /notice modify <index> <message>
	public boolean onModifyCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender.hasPermission("notice.admin")) {
			if (args.length > 2) {
				try {
					int index = Integer.parseInt(args[1]);
					if (index > 0 && index <= plugin.numberOfMessages()) {
						...
						plugin.addToNoticeMessage(message.toString(), index);
						sender.sendMessage(ChatColor.AQUA + "[NOTICE] " + ChatColor.GREEN + "Modified message of the index successfully.");
					} else {
						sender.sendMessage(ChatColor.AQUA + "[NOTICE] " + ChatColor.RED + "Index is out of range.");
					}
				} catch (NumberFormatException e) {
